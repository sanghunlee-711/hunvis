type TimeLine = { desc: string; likes: number };
...
function makeDataGenerator(items: Friend[] | TimeLine[]) {
  //친구목록이나 타임라인이나 생성하는 로직이 같기 때문에 하나의 함수로 작성한다.
  let itemIndex: number = 0;

  return function getNextData() {
    //getNextData() 함수는 items, itemIndex 변수를 기억하는 클로저다.
    const item: Friend | TimeLine = items[itemIndex % items.length];
    itemIndex += 1;
